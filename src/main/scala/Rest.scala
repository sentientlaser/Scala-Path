...
Since a major use case of scala paths is interacting with services, this had been an intention to create an API that treated REST-y calls as functions, so they could easily be composed, etc.
